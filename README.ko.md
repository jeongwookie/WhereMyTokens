...
작업표시줄에 조용히 상주하며 Claude Code와 Codex 사용량 — 토큰, 비용, 세션 활동, 캐시, 모델별 사용량, 속도 제한 — 을 한눈에 보여줍니다.
...
> ⭐ **Claude + Codex 동시 추적** — Settings에서 Claude만, Codex만, 또는 둘 다 추적하도록 선택할 수 있습니다.

> 💾 **클라우드 동기화 없음** — 로컬 Claude/Codex 파일만 읽습니다. 데이터가 외부로 전송되지 않습니다.
...
**[⬇ 인스톨러 다운로드 (.exe)](https://github.com/jeongwookie/WhereMyTokens/releases/download/v1.9.2/WhereMyTokens-Setup.exe)** — 받아서 실행하면 끝
...
1. 릴리즈 페이지에서 `WhereMyTokens-v1.9.2-win-x64.zip` 다운로드
2. 원하는 위치에 압축 해제
...
세션 행의 **Breakdown** 버튼을 클릭하면 카테고리별 활동 분석 패널이 펼쳐집니다. Claude 세션은 output token 배분을 표시하고, Codex 세션은 tool별 output token 대신 function/tool call 로그가 있으므로 tool event count를 표시합니다. 한 번에 하나만 열림.
