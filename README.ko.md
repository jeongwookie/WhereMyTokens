...
브리지 데이터가 활성 상태(5분 이내 업데이트)이면 속도 제한 바가 실시간 값을 사용합니다. Claude Code가 실행 중이 아닐 때는 마지막으로 알려진 API 값으로 폴백합니다.
...
1. [Releases](https://github.com/jeongwookie/WhereMyTokens/releases)에서 `WhereMyTokens-v1.3.3-win-x64.zip` 다운로드
2. ZIP 압축 해제
...
| 우선순위 | 소스 | 설명 |
|---------|------|------|
| 1순위 | **브리지 (stdin)** | `statusLine`을 통해 Claude Code에서 전달되는 실시간 데이터. Claude Code가 API를 호출할 때마다 업데이트. |
| 2순위 | **Anthropic API** | `/api/oauth/usage` — 정확한 % 및 리셋 시간. 3분마다 가져오며 429 시 지수 백오프. |
| 폴백 | **마지막 알려진 값** | API 실패 시 마지막 성공 값 유지. 가져오기 실패로 인해 속도 제한 바가 0으로 리셋되지 않음. |
