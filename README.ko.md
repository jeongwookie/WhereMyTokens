...
<p align="center">
  <a href="https://github.com/jeongwookie/WhereMyTokens/releases/download/v1.11.5/WhereMyTokens-Setup.exe"><strong>v1.11.5 다운로드</strong></a>
  ·
...
**[⬇ 인스톨러 다운로드 (.exe)](https://github.com/jeongwookie/WhereMyTokens/releases/download/v1.11.5/WhereMyTokens-Setup.exe)** — 받아서 실행하면 끝

**[⬇ 포터블 ZIP 다운로드](https://github.com/jeongwookie/WhereMyTokens/releases/download/v1.11.5/WhereMyTokens-v1.11.5-win-x64.zip)** — 설치 없이 실행
...
1. 릴리즈 페이지에서 `WhereMyTokens-v1.11.5-win-x64.zip` 다운로드
2. 원하는 위치에 압축 해제
...
### 분석 & 활동
- **헤더 통계** — today/all-time 토글: 비용, API 호출, 세션, 캐시 효율, 절약 비용, 컴팩트한 Claude/Codex 메타데이터, Claude 폴백/reset 상태를 보여주는 단일 상태 pill
...
헤더 상태 pill은 Claude/API 관련 핵심 상태를 한 곳에 요약합니다. 대표 라벨은 `Local estimate`(로컬 폴백 데이터 사용 중), `Reset unavailable`(사용량은 받았지만 reset 시각이 없음), `Rate limited`, `API offline` 입니다. pill에 마우스를 올리면 최신 상세 사유를 볼 수 있습니다.
...
헤더 상태 pill은 API/폴백 상태를 표시합니다. `Local estimate`, `Reset unavailable`, `Rate limited`, `API offline` 같은 라벨이 보일 수 있고, 마우스를 올리면 최신 상세 사유를 볼 수 있습니다.
