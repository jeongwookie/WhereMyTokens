...
| 版本 | 日期 | 主要变更 |
|------|------|--------|
| **Unreleased** | TBD | 新增 Codex reset credit 作为 Plan Usage target，并支持 Rich/Simple/隐藏显示、auth-bound cache 与 stale/error 状态 |
...
- **按 target 配置 quota 展示** — 每个 provider window 和 model target 都可以在 Settings 中设为 Rich、Simple 或隐藏；这里只影响 Plan Usage 和悬浮小部件展示。Codex Resets target 只在 Plan Usage 中支持 Rich、Simple 或隐藏
