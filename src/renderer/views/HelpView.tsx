  ...
  return (
    <>
      ...
      <Section icon={<Signal size={15} />} title="Startup & Status">
        ...
        <div style={{ marginBottom: 5 }}><B>Header status pill</B> — one pill in the top bar summarizes the most important provider health state and names the affected provider.</div>
      ...
      <Section icon={<Signal size={15} />} title="Data Sources">
        ...
        <SrcRow badge="2nd">
          <B>Limit sources</B> — Claude uses Anthropic API first, then Bridge/Cache fallback. Codex uses live Codex usage first, then Cache/Log fallback.
        </SrcRow>
        <SrcRow badge="FB">
  ...
  return (
    <>
      ...
      <Section icon={<Signal size={15} />} title="시작 상태 & 헤더 표시">
        ...
        <div style={{ marginBottom: 5 }}><B>헤더 상태 pill</B> — 상단 한 개의 pill이 핵심 provider health를 요약하고, 문제가 있는 provider 이름을 함께 표시합니다.</div>
      ...
      <Section icon={<Signal size={15} />} title="데이터 소스">
        ...
        <SrcRow badge="2nd">
          <B>한도 소스</B> — Claude는 Anthropic API를 우선 사용하고 Bridge/Cache로 폴백합니다. Codex는 live Codex usage를 우선 사용하고 Cache/Log로 폴백합니다.
        </SrcRow>
        <SrcRow badge="FB">
  ...
  return (
    <>
      ...
      <Section icon={<Signal size={15} />} title="起動状態とヘッダーステータス">
        ...
        <div style={{ marginBottom: 5 }}><B>ヘッダーステータス pill</B> — 上部の 1 つの pill が重要な provider health をまとめ、影響を受ける provider 名も表示します。</div>
      ...
      <Section icon={<Signal size={15} />} title="データソース">
        ...
        <SrcRow badge="2nd">
          <B>制限ソース</B> — Claude は Anthropic API を優先し、Bridge/Cache にフォールバックします。Codex は live Codex usage を優先し、Cache/Log にフォールバックします。
        </SrcRow>
        <SrcRow badge="FB">
