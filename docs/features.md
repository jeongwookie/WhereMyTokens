...
- Optional draggable Windows taskbar mini quota display with fixed `5h` and `1w` rows, a configurable per-row block limit, transparent background, and taskbar-background-aware text contrast.
...
- Windows-only draggable self-contained taskbar mini display for compact `5H` / `1W` quota rows with a configurable block limit and transparent, sampled-background-aware rendering.
