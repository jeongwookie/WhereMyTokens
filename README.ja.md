...
### レート制限 & アラート
- **Provider quota バー** — Claude、Codex、および今後の provider は `providerQuotas` snapshot で有効な quota を公開します。Claude は Anthropic API/statusLine/cache、Codex は live usage/cache/local-log の優先順位で表示します
...
ネットワークアクセスは有効にした provider チェックボックスの usage endpoint に限定されます。Claude usage polling は最大 5 分ごとに実行し、429 backoff を適用します。Codex live usage は HTTPS-only request、timeout、レスポンスサイズ制限、cache、backoff を適用します。ローカル JSONL 解析と `statusLine` bridge はセッション内容を外部へ送信しません。
