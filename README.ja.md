...
- **2 段階セッショングループ化** — git プロジェクト → ブランチ別にグループ化、プロジェクトごとのコミット・行数統計を表示；idle セッションは段階的に折りたたみ（トップ 3 ツール → コンテキストバーのみ → 6 時間以上は自動非表示）
...
- **ヘッダー統計** — today/all-time トグルでコスト、API 呼び出し数、セッション数、キャッシュ効率 %、キャッシュ節約額、トークン内訳（In/Out/Cache）を一目で表示
- **Code Output** — git ベースの生産性指標：コミット数、純変更行数、**$/100 Lines**（100 行追加あたりのコスト）（today/all time 切り替え）；today は実際の行あたりコストと全期間平均を比較表示；`~/.claude/projects/` の履歴から Claude を使用したすべてのプロジェクトを自動検出 — セッションがアクティブでなくても含まれる
...
- **アクティビティタブ** — 7 日間ヒートマップ、5 ヶ月カレンダー（GitHub スタイル）、時間帯別分布、4 週間比較、**Rhythm**（30 日間の時間帯別コスト分布、ピーク詳細統計）
...
- **Auto/Light/Dark テーマ** — デフォルトはシステム設定に従う；モダン UI、数値表示に JetBrains Mono フォントを使用
...
1. [Releases](https://github.com/jeongwookie/WhereMyTokens/releases) から `WhereMyTokens-v1.9.0-win-x64.zip` をダウンロード
2. ZIP を展開
...
| 表示場所 | 範囲 | tok | $ |
|---------|------|-----|---|
| ヘッダー (today) | 今日の深夜以降 | In/Out/Cache + 呼び出し数、セッション数 | API 換算 + キャッシュ節約 |
| ヘッダー (all) | 全期間 | In/Out/Cache + 呼び出し数、セッション数 | API 換算 + キャッシュ節約 |
...
| Rhythm | 時間帯別コスト分布 — Morning ☀️ / Afternoon 🔥 / Evening 🌆 / Night 🌙、グラデーションバー、ピーク詳細統計（トークン、コスト、リクエスト %）、ローカルタイムゾーン（30 日間） |
