...
ブリッジデータが有効（5 分以内に更新）な場合、レート制限バーはリアルタイムの値を使用します。Claude Code が実行されていない場合は、最後に取得した API 値にフォールバックします。
...
1. [Releases](https://github.com/jeongwookie/WhereMyTokens/releases) から `WhereMyTokens-v1.3.3-win-x64.zip` をダウンロード
2. ZIP を展開
...
| 優先度 | ソース | 説明 |
|--------|--------|------|
| 第 1 | **ブリッジ（stdin）** | `statusLine` 経由で Claude Code からのリアルタイムデータ。Claude Code が API を呼び出すたびに更新。 |
| 第 2 | **Anthropic API** | `/api/oauth/usage` — 正確な % とリセット時間。3 分ごとに取得、429 時は指数バックオフ。 |
| フォールバック | **最後の既知の値** | API 失敗時は最後の成功値を保持。取得失敗でレート制限バーがゼロにリセットされることはない。 |
